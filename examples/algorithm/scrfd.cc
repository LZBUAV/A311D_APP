...
bool SCRFD::post(std::vector<region>& boxes)
{
    region proposal;
            ...
            for (int w = 0; w < fm_w; w++)
            {
                for (int c = 0; c < fm_c; c++)
                {
                    ...
                    if (score > this->score_threshold)
                    {
                        ...
                        proposal.box.w = std::abs(x1 - x0) * this->input_ratio;
                        proposal.box.h = std::abs(y1 - y0) * this->input_ratio;
...
bool SCRFD::Detect(const cv::Mat& image, std::vector<region>& boxes, const float& score_th, const float& nms_th)
{
    if (nullptr == this->graph)
    {
